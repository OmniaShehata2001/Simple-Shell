...
namespace Simple_Shell
{
    public static class Tokenizer
    {
        ...
        private static  List<string> GenerateImport(string [] strsArr)
        {
            List<string> args = new List<string>();
            args.AddRange(strsArr.ToList());
            return args;
        }

        private static  List<Token> GenerateImportToken(List<string> args)
        {
            List<Token> tokens = new List<Token>();
            if (! (args.Count() > 0))
            {
            ...
            var baseToken = generateToken(args.FirstOrDefault(), TokenType.Command);
            tokens.Add(baseToken);
            foreach (var arg in args.Skip(1))
            {
                tokens.Add(generateToken(arg, TokenType.FileName));
            }
            return tokens;
        }
